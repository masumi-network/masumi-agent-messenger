...
|---|---|---|
| `account login` | `[--issuer <url>]`, `[--client-id <id>]`, `[--skip-agent-registration]`, `[--disable-linked-email]`, `[--public-description <text>]`, `[--public-description-file <path>]`, `[--debug]` | Interactive OIDC sign-in and account bootstrap. Agents should use `account login start` and `account login complete` instead. |

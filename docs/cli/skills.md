...
- Prefer `data.readiness.state` for branching when present. `needs_login` means start device-code login; `needs_key_recovery` means run `account sync --json` once if the session is authenticated, then ask the user which path to take if recovery is still required; `ready` means the profile can proceed.
- If `agent list`, thread, or inbox commands return `INBOX_BOOTSTRAP_REQUIRED`, do not retry the same command. Run `masumi-agent-messenger account sync --json` once to create/reconnect local inbox rows, import owned SaaS agents, and adopt an exact matching default private key from another local CLI profile when available, then retry the original command once. If a specific imported agent's public profile description is stale, use the slug from `agent list` and run `masumi-agent-messenger agent update <slug> --public-description "<text>" --json`.
...
- If local private keys are still missing after one `account sync --json`, there are two paths and both require user interaction. Ask the user whether to recover keys from another approved device/backup or approve a destructive key reset. Resetting keys makes old encrypted messages unreadable from that profile.

...
export type AuthFlowOptions = GlobalOptions & {
  issuer?: string;
  clientId?: string;
...
export async function resolveRegistrationSettings(options: AuthFlowOptions) {
  return {
...
async function runBootstrapRefresh(params: {
  ...
  const registration = await resolveRegistrationSettings(params.options);
...
export async function resolveRecoveryFlow(params: {
...
export function shouldOfferBackup(result: AuthenticatedInboxResult): boolean {
  return (
...
export async function maybeOfferAuthBackup(params: {
  result: AuthenticatedInboxResult;
  options: AuthFlowOptions;
  reporter: TaskReporter;
  createdLabel: string;
  ...
  if (!params.options.json && shouldOfferBackup(params.result)) {
    await maybeOfferBackupAfterKeyCreation({
      profileName: params.options.profile,
      reporter: params.reporter,
      promptLabel:
        params.result.keySource === 'rotated'
          ? (params.rotatedLabel ?? 'New inbox keys were created.')
          : params.createdLabel,
    });
  }
}

export function toRecoveredAuthResult(result: BootstrapResult): AuthenticatedInboxResult {
  return {
    authenticated: true,
    expiresAt: null,
    issuer: null,

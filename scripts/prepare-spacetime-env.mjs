...
const GENERATED_RUST_FILE = path.resolve(
  process.cwd(),
  'spacetimedb/src/generated_oidc_config.rs'
);
...
function buildRustStringArray(values) {
  if (values.length === 0) {
    return '&[]';
  }
...
function buildGeneratedRustFile(config) {
  return `// This file is generated by scripts/prepare-spacetime-env.mjs.\n// Re-run \`pnpm run spacetime:prepare-env\` after changing MASUMI_OIDC_ISSUER, MASUMI_OIDC_CLIENT_ID, MASUMI_OIDC_AUDIENCES, MASUMI_NETWORK, SPACETIMEDB_HOST, or SPACETIMEDB_DB_NAME.\n\npub const GENERATED_OIDC_CONFIG_SOURCE: &str = ${JSON.stringify(config.source)};\npub const GENERATED_MASUMI_OIDC_ISSUER: &str = ${JSON.stringify(config.issuer)};\npub const GENERATED_MASUMI_OIDC_CLIENT_ID: &str = ${JSON.stringify(config.clientId)};\npub const GENERATED_MASUMI_OIDC_AUDIENCES: &[&str] = ${buildRustStringArray(config.audiences)};\npub const TRUSTED_OIDC_ISSUERS: &[&str] = &[GENERATED_MASUMI_OIDC_ISSUER];\npub const TRUSTED_OIDC_AUDIENCES: &[&str] = GENERATED_MASUMI_OIDC_AUDIENCES;\npub const GENERATED_MASUMI_NETWORK: &str = ${JSON.stringify(config.network)};\npub const GENERATED_SPACETIMEDB_HOST: &str = ${JSON.stringify(config.spacetime.host)};\npub const GENERATED_SPACETIMEDB_DB_NAME: &str = ${JSON.stringify(config.spacetime.databaseName)};\n`;
}

const action = readCliOption('action') ?? 'prepare-env';
const server = readCliOption('server');
const database = readCliOption('database');
const profileName = readCliOption('profile');
const { env, envSources } = loadWorkspaceEnv(process.cwd());
if (profileName) {
  const profile = PROFILES[profileName];
  if (!profile) {
    throw new Error(
      `Unknown env profile "${profileName}". Expected one of: ${Object.keys(PROFILES).join(', ')}`
    );
  }
  for (const [key, value] of Object.entries(expandProfile(profile))) {
    env[key] = value;
    envSources.set(key, `profile:${profileName}`);
  }
}
...
if (database) {
  const requestedDatabase = canonicalizeSpacetimeDbName(database);
  if (requestedDatabase !== resolvedSpacetimeConfig.databaseName) {
    throw new Error(
      `SPACETIMEDB_DB_NAME mismatch: target database is ${requestedDatabase}, but generated config would use ${resolvedSpacetimeConfig.databaseName}. Switch profiles or pass --profile for the target.`
    );
  }
}
if (server) {
  const { alias } = resolveSpacetimeServerAlias(resolvedSpacetimeConfig.host);
  if (server !== alias) {
    throw new Error(
      `SPACETIMEDB_HOST mismatch: target server is ${server}, but generated config would use ${alias}. Switch profiles or pass --profile for the target.`
    );
  }
}
const generatedFiles = [
  ...
  [GENERATED_RUST_FILE, buildGeneratedRustFile(resolvedOidcConfig)],
];
